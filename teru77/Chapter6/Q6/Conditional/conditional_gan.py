...
for epoch in range(opt.epoch):
    for i, (images, labels) in enumerate(dataloader):
        ...
        g_loss = criterion(validity, reall)
        ...
        d_real_loss = criterion(validity_real, reall)

        # Loss for fake images and labels
        validity_fake = discriminator(gen_imgs.detach(), gen_labels)
        d_fake_loss = criterion(validity_fake, f_label)
    ...
    # 1epochごとに画像を確認(対応するものを生成させる)
    vutils.save_image(images, '%s/real_samples.png' % opt.output, normalize=True)
    fake = generator(noise, labels)
    vutils.save_image(fake.detach(), '%s/fake_samples_epoch_%03d.png' % (opt.output, epoch), normalize=True)

    Loss_D_list.append(d_loss.item())  # Loss_Dデータ
    Loss_G_list.append(g_loss.item())  # Loss_Gデータ
...
plt.grid()
plt.savefig('CganLoss_graph.png')