...
history = model.fit(tf.cast(seq_list_train, tf.float32), y_train, batch_size=32, epochs=epoch_num, verbose=2, validation_split=0.2, shuffle=True, callbacks=[checkpoint])
...
test_loss, test_acc = model.evaluate(tf.cast(seq_list_test, tf.float32), y_test, verbose=0)
...
for i in range(400):
    ...
    # 現在の「文」に続く「字」を予測する
    preds = model.predict(tf.cast(x_pred, tf.float32), verbose=0)[0]
    next_index = np.argmax(preds)
    next_char = idx2char[next_index]
