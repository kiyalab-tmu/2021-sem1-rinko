...
class Generator(nn.Module):
  def __init__(self, input_dim, output_dim):
    ...
    self.main = nn.Sequential(
        nn.ConvTranspose2d(input_dim, 64, 4, 1, 0, bias=False),
        nn.BatchNorm2d(64),
        ...
        nn.ConvTranspose2d(16, output_dim, 2, 2, 2, bias=False),
        nn.Tanh()
    )
...
class Discriminator(nn.Module):
  def __init__(self, input_dim):
    super().__init__()
    self.main = nn.Sequential(
        nn.Conv2d(input_dim, 8, 5, 2, bias=False),
...
data_dim = 1 #データ(MNIST)の次元数

#学習過程で Generatorが生成する画像を可視化するためのノイズz
fixed_z = torch.randn(100, latent_dim, 1, 1, device=device)
...
def D_train(x):
  ...
  # 2: 入力が偽物のデータのとき
  z = torch.randn(x.size(0), latent_dim, 1, 1, device=device)
  y_pred = netD(netG(z))
