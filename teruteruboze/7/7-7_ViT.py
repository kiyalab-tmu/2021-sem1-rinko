...
if __name__ == '__main__':
    ...
    transform = transforms.Compose([
                transforms.Resize(config['training']['img_size']),
                transforms.CenterCrop(config['training']['img_size']),
                transforms.ToTensor(),
                transforms.Normalize([0.5], [0.5], [0.5])
                ])

    dataset = makeDataset.BasicDatasetCIFAR10(
        config['training']['BATCH_SIZE'],
        config['path']['dataset'],
        transforms=transform,
        isVALID=config['dataset']['num_valid'])
    dataset.add_class_label(config['dataset']['class_name'])
