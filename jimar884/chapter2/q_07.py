...
def average_pooling(img):
    ...
    for h in range(0, heignt, 8):
        for w in range(0, width, 8):
            result_img[h:h+8, w:w+8, 0] = np.mean(result_img[h:h+8, w:w+8, 0])
            result_img[h:h+8, w:w+8, 1] = np.mean(result_img[h:h+8, w:w+8, 1])
            result_img[h:h+8, w:w+8, 2] = np.mean(result_img[h:h+8, w:w+8, 2])
